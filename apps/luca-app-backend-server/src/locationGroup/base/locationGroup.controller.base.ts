/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import * as common from "@nestjs/common";
import * as swagger from "@nestjs/swagger";
import { isRecordNotFoundError } from "../../prisma.util";
import * as errors from "../../errors";
import { Request } from "express";
import { plainToClass } from "class-transformer";
import { ApiNestedQuery } from "../../decorators/api-nested-query.decorator";
import { LocationGroupService } from "../locationGroup.service";
import { LocationGroupCreateInput } from "./LocationGroupCreateInput";
import { LocationGroup } from "./LocationGroup";
import { LocationGroupFindManyArgs } from "./LocationGroupFindManyArgs";
import { LocationGroupWhereUniqueInput } from "./LocationGroupWhereUniqueInput";
import { LocationGroupUpdateInput } from "./LocationGroupUpdateInput";

export class LocationGroupControllerBase {
  constructor(protected readonly service: LocationGroupService) {}
  @common.Post()
  @swagger.ApiCreatedResponse({ type: LocationGroup })
  async createLocationGroup(
    @common.Body() data: LocationGroupCreateInput
  ): Promise<LocationGroup> {
    return await this.service.createLocationGroup({
      data: {
        ...data,

        company: data.company
          ? {
              connect: data.company,
            }
          : undefined,
      },
      select: {
        company: {
          select: {
            id: true,
          },
        },

        createdAt: true,
        id: true,
        updatedAt: true,
      },
    });
  }

  @common.Get()
  @swagger.ApiOkResponse({ type: [LocationGroup] })
  @ApiNestedQuery(LocationGroupFindManyArgs)
  async locationGroups(
    @common.Req() request: Request
  ): Promise<LocationGroup[]> {
    const args = plainToClass(LocationGroupFindManyArgs, request.query);
    return this.service.locationGroups({
      ...args,
      select: {
        company: {
          select: {
            id: true,
          },
        },

        createdAt: true,
        id: true,
        updatedAt: true,
      },
    });
  }

  @common.Get("/:id")
  @swagger.ApiOkResponse({ type: LocationGroup })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  async locationGroup(
    @common.Param() params: LocationGroupWhereUniqueInput
  ): Promise<LocationGroup | null> {
    const result = await this.service.locationGroup({
      where: params,
      select: {
        company: {
          select: {
            id: true,
          },
        },

        createdAt: true,
        id: true,
        updatedAt: true,
      },
    });
    if (result === null) {
      throw new errors.NotFoundException(
        `No resource was found for ${JSON.stringify(params)}`
      );
    }
    return result;
  }

  @common.Patch("/:id")
  @swagger.ApiOkResponse({ type: LocationGroup })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  async updateLocationGroup(
    @common.Param() params: LocationGroupWhereUniqueInput,
    @common.Body() data: LocationGroupUpdateInput
  ): Promise<LocationGroup | null> {
    try {
      return await this.service.updateLocationGroup({
        where: params,
        data: {
          ...data,

          company: data.company
            ? {
                connect: data.company,
              }
            : undefined,
        },
        select: {
          company: {
            select: {
              id: true,
            },
          },

          createdAt: true,
          id: true,
          updatedAt: true,
        },
      });
    } catch (error) {
      if (isRecordNotFoundError(error)) {
        throw new errors.NotFoundException(
          `No resource was found for ${JSON.stringify(params)}`
        );
      }
      throw error;
    }
  }

  @common.Delete("/:id")
  @swagger.ApiOkResponse({ type: LocationGroup })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  async deleteLocationGroup(
    @common.Param() params: LocationGroupWhereUniqueInput
  ): Promise<LocationGroup | null> {
    try {
      return await this.service.deleteLocationGroup({
        where: params,
        select: {
          company: {
            select: {
              id: true,
            },
          },

          createdAt: true,
          id: true,
          updatedAt: true,
        },
      });
    } catch (error) {
      if (isRecordNotFoundError(error)) {
        throw new errors.NotFoundException(
          `No resource was found for ${JSON.stringify(params)}`
        );
      }
      throw error;
    }
  }
}
